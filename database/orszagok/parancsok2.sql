...
SELECT MAX(nepesseg*1000) AS maxnep, orszag  FROM `orszagok` WHERE nepesseg = (SELECT MAX(nepesseg) FROM orszagok);




/*40. Melyik a legkisebb népességű ország, és hány lakosa van?*/
SELECT MIN(nepesseg*1000) AS minnep, orszag  FROM `orszagok` WHERE nepesseg = (SELECT MIN(nepesseg) FROM orszagok);
...
SELECT terulet AS maxafrika, orszag  FROM `orszagok` WHERE foldr_hely LIKE "%Afrika%" ORDER BY `maxafrika` DESC LIMIT 1;

/*44. Melyik a legkisebb amerikai ország és hányan lakják?*/
SELECT terulet AS minafrika, orszag  FROM `orszagok` WHERE foldr_hely LIKE "%Amerika%" ORDER BY `minafrika` ASC LIMIT 1;

/*45. Melyik az első három legsűrűbben lakott "országméretű" ország (tehát nem város- vagy törpeállam)?*/
SELECT nepesseg*1000/terulet AS megsurubben, orszag  FROM `orszagok` WHERE foldr_hely NOT LIKE "%törpeállam%" AND foldr_hely NOT LIKE "%városállam%" ORDER BY `megsurubben` DESC LIMIT 3;
...
SELECT ABS(terulet-(SELECT terulet FROM orszagok WHERE orszag = "Magyarország")) AS abs2, orszag FROM `orszagok`
WHERE orszag != "Magyarország"
ORDER BY abs2 ASC LIMIT 3;
