var bookss = [
  ...
  window.onload = function createCards() {
    bookdiv = document.getElementById("books-div");
    pages = []
    pageContent = [];
    count = 0;
    customid = 0;
    bookss.forEach(book => {
        pageContent.push(book);
        count++;
        if(count == 5) {
            count = 0;
  ...
  function pageOpen(id) {
    var bookdiv = document.getElementById("books-div");
    bookdiv.innerHTML = "";
    var index = 0;
    pages[id].forEach(book => {
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-body">
                Szerző: ${book.author}<br>
                Cím: ${book.title}<br>
                Megjelenés éve: ${book.year}<br>
                Oldalak száma: ${book.pages}<br>
                Nyelv: ${book.language}
